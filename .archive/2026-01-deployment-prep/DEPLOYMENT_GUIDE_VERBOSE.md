...
**Archivo de configuración:** `terraform/terraform.prod.tfvars` (ya existe)

```bash
# Revisar configuración ARM existente
cat terraform/terraform.prod.tfvars
```

**Configuración actual (ARM64):**

```hcl
# Ya configurado para ARM64
architecture = "arm"   # CAX series - ARM (Ampere Altra Q80-30)
server_size  = "small" # Auto-selects: cax11 (2 vCPU ARM64, 4GB RAM)
location     = "nbg1"  # Nuremberg datacenter

# Personalizar según necesites:
ssh_allowed_ips = ["TU.IP.PUBLICA/32"]
domain = "tudominio.com"
enable_cloudflare = true  # Si usas Cloudflare
```

**Nota:** El archivo `terraform.prod.tfvars` ya está optimizado para producción ARM64. Solo necesitas ajustar IPs y dominio.
...
# Ver plan de ejecución con tfvars de producción
...
# Exportar token de Hetzner Cloud
export HCLOUD_TOKEN="tu_token_hetzner"

# Verificar que el plugin detecta servidores
ansible-inventory -i inventory/hetzner.hcloud.yml --list

# Ver servidores agrupados
ansible-inventory -i inventory/hetzner.hcloud.yml --graph

# Test de conectividad (usa ansible.cfg automáticamente)
ansible all -m ping
```

**Nota:** El inventario dinámico detecta automáticamente todos los servidores de tu cuenta Hetzner usando labels de Terraform (`environment=production`).
