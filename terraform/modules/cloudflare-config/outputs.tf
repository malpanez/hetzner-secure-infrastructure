...
output "extra_subdomain_record_ids" {
  description = "Map of extra subdomain name to DNS record ID"
  value       = { for name, record in cloudflare_record.extra_subdomains : name => record.id }
}
...
output "configuration_summary" {
  ...
  value = {
    ...
    dns_records_count = 2 + length(var.extra_subdomains)
    rulesets_count    = 4
