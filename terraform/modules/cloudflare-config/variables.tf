...
variable "extra_subdomains" {
  description = "Extra subdomains (A records, proxied) pointing to server_ipv4 (e.g., [\"grafana\", \"prometheus\"])"
  type        = list(string)
  default     = []

  validation {
    condition     = alltrue([for s in var.extra_subdomains : can(regex("^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$", s))])
    error_message = "Each subdomain must be a valid DNS label (lowercase alphanumeric and hyphens)"
  }
}

variable "media_path_bic_off" {
  description = "URI path prefix (e.g., \"/media/\") where Browser Integrity Check and challenges are disabled, for non-browser/native clients fetching media. Empty string disables the rule."
  type        = string
  default     = ""
}
...
variable "monitoring_access_emails" {
  description = "Emails allowed through Cloudflare Access to grafana.<domain> and prometheus.<domain>. When non-empty, Access apps + allow policies are created for both subdomains."
  type        = list(string)
  default     = []
}
