...
resource "cloudflare_record" "root" {
  ...
  comment = "Root domain pointing to origin server"
}
...
# Extra subdomains pointing to the same origin server (data-driven)
# e.g. extra_subdomains = ["grafana", "prometheus", "blog"]
resource "cloudflare_record" "extra_subdomains" {
  ...
  proxied = true
  ttl     = 1
  comment = "Subdomain ${each.value} pointing to origin server (Nginx vhost / reverse proxy)"
}
