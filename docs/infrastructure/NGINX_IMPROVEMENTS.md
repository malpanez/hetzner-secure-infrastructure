...
> ✅ IMPLEMENTED — this was a pre-implementation proposal and has since been
> delivered. See NGINX_MODULAR_IMPLEMENTATION.md
> for the deployed modular config. The notes below are kept for rationale/history.
...
# HSTS: set at the Cloudflare edge via the security-headers ruleset
# (terraform/modules/cloudflare-config/rulesets.tf), so it is NOT added here
# to avoid a duplicate header. If serving direct SSL (no Cloudflare), add:
