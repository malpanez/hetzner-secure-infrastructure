...
```bash
git clone https://github.com/YOUR-USERNAME/hetzner-secure-infrastructure.git
...
```bash
git remote add upstream https://github.com/malpanez/hetzner-secure-infrastructure.git
```
...
make test-terraform
make test-ansible
make test-molecule-role ROLE=nginx_wordpress
```
...
```bash
# Format and validate (OpenTofu; local binary is `tofu`)
cd terraform
tofu fmt -recursive
tofu validate
...
variable "server_type" {
  ...
  validation {
    condition     = contains(["cax11", "cax21", "cax31"], var.server_type)
    error_message = "Server type must be cax11, cax21, or cax31."
  }
}
...
def deploy_infrastructure(server_type: str, region: str) -> dict[str, any]:
    ...
    Args:
        server_type: The Hetzner server type (e.g., 'cax11')
...
- **Issues**: [Project Issues](https://github.com/malpanez/hetzner-secure-infrastructure/issues)
