...
This section describes how to transition deployment users (like `deploy`) from break-glass access to full 2FA after infrastructure is deployed and verified.

### Current Setup (During Deployment)

The `deploy` user is configured as a break-glass user for safe deployment:

```yaml
ssh_2fa_break_glass_enabled: true
ssh_2fa_break_glass_users:
  - deploy  # Deployment user, no 2FA during setup
```
...
- **Separate from admin users**: `deploy` stays in built-in `sudo` group for admin access
...
- `deploy` user keeps existing `sudo` group membership (unaffected)
...
**Step 2: Provision 2FA for deploy**
...
ssh-keygen -t ed25519-sk -O resident -O verify-required -f ~/.ssh/id_fido2_deploy

# On server: Provision YubiKey OATH for sudo
sudo /usr/local/bin/provision-yubikey-oath.sh deploy
sudo usermod -aG mfa-required deploy
```
...
```yaml
# Remove deploy from break-glass list
ssh_2fa_break_glass_users: []
...
```bash
# Check SSH config
ssh deploy@server 'sudo grep -A20 "Match All" /etc/ssh/sshd_config.d/50-2fa.conf'
# Should show: AuthenticationMethods publickey,keyboard-interactive

# Verify deploy NOT in break-glass group
